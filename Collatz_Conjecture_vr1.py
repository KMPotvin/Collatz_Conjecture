...
class Application(tk.Frame):
    ...
    def click(self):
        ...
        num_list = collatz_conjecture(workingNum)
        iterations = len(num_list)
        maxi = max(num_list)
        self.return_entry.config(text=f'{iterations} cycles is what it took to reach the 4-2-1 loop. Highest peak was {maxi}.')
    ...
    def createwidgets(self):
        ...
        self.return_entry = tk.Label(self, fg='green', bg='black', width=60)
        self.return_entry.grid(row=8, columnspan=4)
        self.return_entry['font'] = centurygoth36

        self.tryagainButton = tk.Button(self, text='Try Again!', width=20, command=self.cleartext, bg='green', fg='white')
        self.tryagainButton.grid(row=17, column=0, sticky=tk.W)
